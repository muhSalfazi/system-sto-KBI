...
@endif
  <section class="section">
      ...
      <div class="card-body">
        ...
        <form class="row g-3 needs-validation" novalidate enctype="multipart/form-data" method="POST" action="{{ route('reports.update', $report->id) }}">
          @csrf
          @method('PUT')
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <select name="status" id="status" class="form-select">
                  <option value="OK" {{ old('status', $report->status ?? '') == 'OK' ? 'selected' : '' }}>OK</option>
                  <option value="NG" {{ old('status', $report->status ?? '') == 'NG' ? 'selected' : '' }}>NG</option>
                <option value="VIRGIN" {{ old('status', $report->status ?? '') == 'VIRGIN' ? 'selected' : '' }}>VIRGIN</option>
                <option value="FUNSAI" {{ old('status', $report->status ?? '') == 'FUNSAI' ? 'selected' : '' }}>FUNSAI</option>
              </select>
            </div>
          </div>
